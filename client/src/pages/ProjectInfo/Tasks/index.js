    ...
    },
    {
      ...
      render:(text,record)=>{
        return (<select value={record.status} 
          onChange={(e)=>{
            onStatusUpdate({task:record,status:e.target.value,});
          }}
      ...
      render:(text,record)=>{
        return(
          <div className="flex gap-2">
            <Button type="primary" onClick={()=>{setTask(record); setShowTaskForm(true);}}>Edit</Button>
    ...
    <div>
      ...
      <div className="flex gap-5">
        ...
        <div>
          ...
          >
            <option value="all">All</option>
             {project.members.filter((m)=>m.role==='admin' ||  m.role === 'owner').map((m)=>(
              <option value={m.user._id}>{m.user.firstName+" "+m.user.lastName}</option>
             ))}
          </select>
        </div>
        ...
        <div>
          ...
          >
            <option value="all">All</option>
             {project.members.filter((m)=>m.role==='employee').map((m)=>(
              <option value={m.user._id}>{m.user.firstName+" "+m.user.lastName}</option>
             ))}
          </select>
        </div>
      ...
      {showViewTask && (
        ...
        >
          ...
          {task.attachments.map((image)=>{
            return (<img src={image} alt="" className="w-40 h-40 object-cover mt-2 p-2 border border-solid rounded border-gray-500"/>);
          })}
          </div>
